...
function DeleteDate()
{
	...
	if( DBOpen() ) {
		$Result = DBDo( "DELETE FROM ".TAB_DATES." WHERE ID='".$_REQUEST['dateid']."'" );
		DBClose();
	}// if
	else {
...
function CancelDate()
{
	...
	if( DBOpen() ) {
		$Result = DBDo( "SELECT * FROM ".TAB_DATES." WHERE ID='".$_REQUEST['dateid']."'" );
		if( $Result ) {
			$Row = DBFetchArray();
		}// if
...
function StoreSettings()
{
	...
	if( DBOpen() ) {
		$Result = DBDo(   "UPDATE ".TAB_SETTINGS. " SET value='"
                               . $Num
...
{
	if( DBOpen() ) {
		$result = DBDo( "SELECT * FROM ".TAB_DATES." WHERE id=".$_REQUEST['date'] );
		$RowTime = DBFetchArray();
...
function EnterNewDate()
{
	...
	if( DBOpen() ) {
		$Result = DBDo( "SELECT * FROM ".TAB_LOCATIONS." WHERE ID='".$_REQUEST['location']."'" );
		if( $Result ) {
...
function DeleteLocation()
{
	...
	if( DBOpen() ) {
		$Result = DBDo( "DELETE FROM ".TAB_LOCATIONS." WHERE ID='".$_REQUEST['locationid']."'" );
		DBClose();
	}// if
	else {
		$Output .= 'Could not access DB';
...
function CheckNewLocation()
{
    ...
    if( DBOpen() ) {
		$Result = DBDo( "SELECT * FROM ".TAB_LOCATIONS." WHERE name='".$_REQUEST['locationname']."'" );
		DBClose();
...
function FormLocation( $Action, $Name, $Address, $Remarks, $ID )
{
	...
	if( $Action != 'enternewlocation' ) {
        ...
        $Content['BUTTONDELETE'] = ' <input type="submit" name="btnsend" value="Delete"/>';
	}// if
...
function ListLocations() 
{
	if( DBOpen() ) {
		$LocalOutput = '';
		$Result = DBDo( "SELECT * FROM ".TAB_LOCATIONS." ORDER BY name ASC" );
		...
		while( $Row = DBFetchArray()) {
			...
			$Remarks = htmlentities( DBEscapeString( $Row['remarks'] ) );
			$Remarks = str_replace ('\n', '<br>', $Remarks);
...
function ListDates( $Past )
{
	...
	if( DBOpen() ) {
		...
		$LocalOutput = '';
		$result = DBDo( "SELECT * FROM ".TAB_DATES." ORDER BY timestamp ASC" );
