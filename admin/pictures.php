...
function PictureDelete()
{
	...
	if( $_REQUEST['file']!='') {
		if( @file_exists( PIC_FILE_ROOT.PIC_IMAGE_DIR.$_REQUEST["file"] ) == true ) {
			...
			//Delete DB entry
			if( DBOpen() ) {
				DBDo( "DELETE FROM ".TAB_PICTURES." WHERE name='".$_REQUEST["file"]."'" );
				DBClose();
			}// if
...
function PicturesUpdate()
{
	...
	if( DBOpen() && $fp ) {
		...
		while (false !== ($file =readdir($fp) ) ) {
			if( filetype( PIC_FILE_ROOT.PIC_IMAGE_DIR.$file ) != "dir" ) {
				$Result = DBDo( "SELECT * FROM ".TAB_PICTURES." WHERE name='".$file."'" );
				if( $Result ) {
					if( !DBGetNumRows() ){
	...
	if( DBOpen() ) {
		if( $FreePosition != false ) {
			$Result = DBDo( "SELECT * FROM ".TAB_PICTURES );
			if( $Result ) {
				while( $Row = DBFetchArray() ) {
					if( !file_exists( PIC_FILE_ROOT.PIC_IMAGE_DIR.$Row['name'] ) ) {
						DBDo( "DELETE FROM ".TAB_PICTURES." WHERE name='".$Row['name']."'" );
...
function PicturesRenumber() 
{
	...
	if( DBOpen() ) {
		...
		if( $Result ) {
			while( $Row = DBFetchAssoc($Result) ) {
				DBDo( "UPDATE ".TAB_PICTURES." SET position=".$FreePosition." WHERE id=".$Row['id'] );
				$FreePosition++;
			}// while
		}// if
		DBClose();
...
function PicturesGetFirstFreePosition() 
{
	...
	if( DBOpen() ) {
		$Result = DBDo( "SELECT MAX(position) AS position FROM ".TAB_PICTURES );
		if( $Result ) {
			$Row = DBFetchArray();
...
function PictureMoveTop( $name ) 
{
	$FreePosition = PicturesGetFirstFreePosition();
	if( DBOpen() ) {
		DBDo( "UPDATE ".TAB_PICTURES." SET position=0 WHERE name='".$name."'" );
		DBClose();
	}// if
	PicturesRenumber();
...
{
	if( DBOpen() ) {
		$Result = DBDo( "SELECT * FROM ".TAB_PICTURES." WHERE name='".$name."'" );
		if( $Result ) {
			if( $Row = DBFetchAssoc( ) ) {
				if( $Row['position'] > 1 ) {
					DBDo( "UPDATE ".TAB_PICTURES." SET position=".$Row['position']." WHERE position=".($Row['position']-1) );
					DBDo( "UPDATE ".TAB_PICTURES." SET position=".($Row['position']-1)." WHERE name='".$name."'" );
				}// if
			}// if
		}// if
		DBClose();
...
function PictureMoveBottom( $name ) 
{
	$FreePosition = PicturesGetFirstFreePosition();
	if( DBOpen() ) {
		DBDo( "UPDATE ".TAB_PICTURES." SET position=".$FreePosition." WHERE name='".$name."'" );
		DBClose();
	}// if
