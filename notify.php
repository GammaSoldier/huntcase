...
function NotifyUsers( $Test = false )
{
	...
	if( DBOpen() ) {
		$Result = DBDo( "SELECT * FROM ".TAB_USERS );
                ...
                if( $Test ) {
                    ...
                    for( $j = 0; $j < $i; $j++ ) {
                       if( ($FarestDate - $Row[$j]['notified']) >= DAYS_IN_ADVANCE * 24 * 3600 ) {
                            DBDo( "UPDATE ".TAB_USERS." SET notified=".$Now." WHERE id=".$Row[$j]['id'] );
                            $BCC .= $Row[$j]['email'].',';
                        }// if
                    }// for $j
                }// else
...
function GenerateText( $mime_boundary, $Now, &$FarestDate, $DaysInAdvance = 5 )
{
	...
	$PlainDate = '';
	if( DBOpen() ) {
		$Result = DBDo( "SELECT * FROM ".TAB_DATES." WHERE isnewsletter=1 ORDER BY timestamp ASC" );
